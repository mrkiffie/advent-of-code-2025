...
fn solve(input: &str, find: usize) -> usize {
    let nodes = parse_input(input);

    let mut distances = HashMap::<Tuple, i64>::with_capacity(nodes.len() * nodes.len());
    ...
    for (Tuple(a, b), _) in &sorted[..find] {
    ...
    let mut counts: Vec<usize> = circuits.values().map(Vec::len).collect();
